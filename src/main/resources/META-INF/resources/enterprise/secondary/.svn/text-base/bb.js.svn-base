...
var add_equip = ["add_rlxx","add_cpxx","add_pfk","add_zbfdz","add_zbfdz_md","add_guolu_md","add_yaolu_md","add_yfl","add_yfl_md","add_rjsy","add_yjytcg","add_yjytzz","add_ltdc","add_yfl_yl","add_yfl_fl","add_yfl_cp"];//添加设备按钮
$(function (){
				...
				if(data.data.表comp09_原辅料及产品信息表v原料信息.length > 0 ) {
					for ( var j = 0 ; j < data.data.表comp09_原辅料及产品信息表v原料信息.length; j ++ ) {
						...
						$.each(data.data.表comp09_原辅料及产品信息表v原料信息[j],function(i,g) {
							$("#yfl_yl"+index+" #"+i).editable("setValue",g);
						});
					}
				}
	...
	$("#tab_9").find(".editable").each(function(el,n){
		...
		if(obj.is(':visible')){
			...
			if(html_id == "equipId"){
				...
				if (html_value == "" || html_value == undefined ) {
					check_Result.push("体积(表9)");
				}
			}else if ( html_id == "height" ) {
				if (html_value == "" || html_value == undefined ) {
					check_Result.push("高度(表9)");
				}
			}else if ( html_id == "diameter" ) {
				if (html_value == "" || html_value == undefined ) {
					check_Result.push("直径(表9)");
				}
			}else if ( html_id == "fuelunit" ) {
				if (html_value == "" || html_value == undefined ) {
