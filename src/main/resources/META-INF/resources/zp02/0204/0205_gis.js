...
function space_dingwei(){
	...
	app.shi.setDefinitionExpression("ADMINCODE <> '"+parent.dataBase.Message_map.REGION+"'");
	...
	app.shi.setRenderer(new dong.SimpleRenderer(symbol));
}
...
function  point() {
	var data ;
	if($('input[name="yonghuname"]:checked').val()=="市填报用户"){
		...
		data = ajax_async_t(BackstageIP+"taskDataFillChild/selectLonAndLatCom",{userId:dataBase.Login_map.SOLE},"JSON","1","POST");
	}
	if( data != "" && data != null && data != undefined ){
		if ( data.status == "success" ) {
			app.gLyr.clear();
			app.shi.clear();
			app.map.removeLayer(app.shi);
			app.shi = new  dong.FeatureLayer(ArcGisServerUrl+"/arcgis/rest/services/polist/cms/MapServer/1", {mode:esri.layers.FeatureLayer.MODE_ONDEMAND,outFields: ["*"]});
			app.map.addLayer(app.shi);
...
var code_tongguo = [];//在当前区域内
var code_bohu = [];//不在当前区域内

var tongguo_companyId = [];//通过 ——市填报companID  上传文件里面的ID
var bohu_companyId = [];//未通过-市填报companID  上传文件里面的ID
...
function test(){
	...
	var query = new dong.Query();
	query.where =  "ADMINCODE in ('"+parent.dataBase.Message_map.REGION+"')";
	app.shi.queryFeatures(query, function(extent) {
         app.map.setMapCursor("default");
         var graphics = app.gLyr.graphics;
         console.log(graphics);
         for(var i= 0, total=graphics.length;i<total;i++){
             if(extent.features[0].geometry.contains(graphics[i].geometry)){
                 selectedGraphics.push(graphics[i]);
                 graphics[i].symbol = createSymbol(iconPath,"#0000EE");
                 app.gLyr.redraw();
                 code_tongguo.push(graphics[i].attributes.id);
                 if ($('input[name="yonghuname"]:checked').val()=="市填报用户"){
                	 tongguo_companyId.push(graphics[i].attributes.companyId)
                 }
             } else {
            	 code_bohu.push(graphics[i].attributes.id);
            	 if ($('input[name="yonghuname"]:checked').val()=="市填报用户"){
            		 bohu_companyId.push(graphics[i].attributes.companyId)
                 }
             }
...
function space_zhenduan(){
	var ste = {}
	if ($('input[name="yonghuname"]:checked').val()=="市填报用户"){
		ste.pass=code_tongguo;
		ste.reject=code_bohu;
		ste.passCompanyId=tongguo_companyId;
		ste.rejectCompanyId=bohu_companyId;
