...
var reg_year=/^(?:(?!0000)[0-9]{4}-(?:(?:0[1-9]|1[0-2])))$/; 
...
function validate_3_2(v){
	...
	if((!v.match(reg_year))&&v) return '请按照YYYY-MM格式填写';
}
...
function validate_4(v){
	...
	var age = parseInt(v);
	var date ;
	if(window.sessionStorage.getItem("year") == "" ||window.sessionStorage.getItem("year")== null ) {
		date = new Date().getFullYear();
	} else {
		date = window.sessionStorage.getItem("year");
	}
