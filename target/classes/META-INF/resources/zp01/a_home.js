...
//					html += '<td><a onclick="edit_task('+item.id+',\'update\','+item.goalYear+')">编辑</a>&nbsp;&nbsp;&nbsp;<a onclick="tijiao('+item.id+')">提交</a></td></tr>';
					...
					html += '<td><a onclick="look_task('+item.id+',\'look\','+item.goalYear+','+item.dataLevel+',\''+ item.status +'\')">查看</a></td></tr>';
				}
				k += 34;
			});
	...
	var zhanwei_height = document.body.clientHeight-(717+k)
	if(zhanwei_height>0){
...
//删除照片
function pic_del(id,url,delUrl){
            ...
            if(data.data.length == 1 && (data.data[0].v1 == 'text')){
                ...
                $.each(data.data,function(i,item){
                    tupian_html += "<li id=\"pin_li_"+item.id+"\"><p class=\"imgWrap\"><a href=\"../gg/"+item.realUrl+"\" title=\"现场照片\" data-gallery=\"\"  target='_blank'>" +
                        ...
                        "<div id=\"pin_del_"+item.id+"\" class=\"file-panel\" style=\"height: 0px;\"><span class=\"cancel\" onclick='pic_del("+item.id+",\""+url+"\",\""+ delUrl +"\");'>删除</span></div></li>";
                });
...
function edit_task(task_id,update,year,level){
	...
	if ( level == "1" ) return window.location.href="0101/b.html?sole="+seesion_sole;
	else if (level == "2" ) return window.location.href="0102/bb.html?sole="+seesion_sole;
	else if ( level == "3" ) return window.location.href="0103/b.html?sole="+seesion_sole;
	
}
//查看任务
function look_task(task_id,look,year,level,status){
    window.sessionStorage.setItem("taskId",task_id);
	window.sessionStorage.setItem("type",look);
	window.sessionStorage.setItem("year",year);
	if(status == '未调查'){
        var data = ajax_async_t(BackstageIP+'taskDataFillChild/selectReason',{taskId:task_id},"json");
        if (data!= "" && data != undefined && data != null ) {
            if ( data.status == "success" ) {
                $("#dialog-yuanyin").dialog("open");
                if(data.data.length == 1 && (data.data[0].v1 == 'text')){
                	...
                	$('#showImg').attr('src',"../gg/"+ data.data[0].realUrl)
				}
            }
        }
	}else{
        if( level == "1" ) window.location.href="0101/b.html?sole="+seesion_sole;
        else if ( level == "2" ) window.location.href="0102/bb.html?sole="+seesion_sole;
        else if ( level == "3" ) window.location.href="0103/b.html?sole="+seesion_sole;
    }
}
...
function tijiao(taskId,str){
	...
	tijiao_str = str;
	$("#dialog-chakan-content").html("是否提交当前信息");
...
function sure_tijiao (){
	var data = ajax_async_t(BackstageIP+"taskDataFillChild/updateCompanyTaskStatus.do",{userId:userId,version:version,id:jitiao_taskId,dataType:dataBase.Message_map.DATA_LEVEL},"json");
	if ( data != "" && data != null && data != undefined ){
		if (data.status == "success") {
			renwu();
